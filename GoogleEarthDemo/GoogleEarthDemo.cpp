...
void GoogleEarthDemo::resizeEvent(QResizeEvent* event)
{
	...
	//大小对不上 临时方案 放缩一下
	qDebug() << "Resize:" << MoveWindow(mainHandle, 0, 30, rect.width()+270, rect.height()+130, false);
...
void GoogleEarthDemo::OnBtnChangeClicked()
{
	...
	if (NULL != pointInfo)
	{
		ui.edtLtd->setText(QString::number(pointInfo->Longitude()));
		ui.edtStd->setText(QString::number(pointInfo->Latitude()));
		ui.edtAtd->setText(QString::number(pointInfo->Altitude()));
